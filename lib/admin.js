    ...
    if (isStreetAddress && (row.Sector === 'Residential' || row.Sector === 'Agriculture')) {
      geocoder.search({ZIP: row.Zip.substring(0, 5)}, function(row, err, res) {
        if (res.candidates.length) {
          row.display_point = self.makePoint(res.candidates)
        } else {
          row.display_point = ""
        }
        next(null, row)
    ...
    data.forEach(function(row) {
      ...
      if (typeof row[region_key] != 'undefined' && row[region_key] != '') {
        ...
        if (region) {
          ...
          if (intersections.length > 1) data = data.concat(self.makeRegionalRows(row, intersections, 'zipcode'))
          intersections = self.findRegionInRegion(region, counties, 'county')
          if (intersections.length > 1) data = data.concat(self.makeRegionalRows(row, intersections, 'county'))
          intersections = self.findRegionInRegion(region, legislative, 'legislative')
          if (intersections.length > 1) data = data.concat(self.makeRegionalRows(row, intersections, 'legislative'))
          intersections = self.findRegionInRegion(region, congressional, 'congressional')
          if (intersections.length > 1) data = data.concat(self.makeRegionalRows(row, intersections, 'congressional'))
        }
      } else {
        row.regional = false
      }
